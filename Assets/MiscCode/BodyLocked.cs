...
namespace Hecomi.HoloLensPlayground
{
    ...
    public class BodyLocked : MonoBehaviour
    {
        ...
        public float overrideDistans = 0;
        ...
        void LateUpdate()
        {
            ...
            if (overrideDistans != 0)
            {
                overrideDistans = Mathf.Clamp(overrideDistans, minDistance, maxDistance);
                targetDistance_ = overrideDistans;
            }

            distance_ = Mathf.Max(distance_ + (targetDistance_ - distance_) * distanceSmoothness, minDistance);

            var scaleFactor = distance_ / maxDistance;
            ...
            transform.localScale = initScale_ * Mathf.Clamp(scaleFactor,minScaleRatio,1.0f);
        }
    }
